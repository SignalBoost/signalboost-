...
async function getInitialLanguage() {
  ...
  if (hasManualLanguageChoice() && isSupportedLocale(saved)) {
    return saved;
  }
  ...
  const browser = navigator.languages?.[0] || navigator.language || null;
...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  ...
  const setLang = async (newLang: string) => {
    ...
    const safeLang = SUPPORTED_LOCALES.includes(normalized) ? normalized : "en";
